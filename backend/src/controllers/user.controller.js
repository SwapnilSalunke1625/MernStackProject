...
const giveFeedback = asyncHandler(async (req, res) => {
  ...
  if (stars === undefined || stars === NaN || stars === '') {
    throw new ApiError(500, 'Star Rating is undefined or Not a Number or empty');
  }
  if (content === undefined || content === '') {
    throw new ApiError(500, 'Empty or undefined content provided during feedback');
  }
  ...
  const createdFeedback = Feedback.findById(feedback._id);
  if (!createdFeedback) {
    throw new ApiError(400, 'Something went wrong while creating the feedback.')
  }

  return res.status(200).json(new ApiResponse(200, 'Feedback given successfully'));
});

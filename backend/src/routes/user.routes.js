...
router.route('/update-user-details').put(verifyJWT, upload.fields(
    [
        { name: "avatar", maxCount: 1 }, { name: "coverImage", maxCount: 1 }
    ]
), updateUserDetails)
...
router.route('/:jobId/userConsent').patch(verifyJWT, userConsentForJobCompletionSetTrue);
router.route('/:jobId/SPConsent').patch(verifyJWT, serviceProviderConsentForJobCompletionSetTrue);
